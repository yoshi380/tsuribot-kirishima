...
async function getWaveHeight(targetDateStr) {
  ...
  const isoDate = `${targetDateStr}T06:00:00+09:00`; // JST朝6時
...
async function getFishingInfo(isTomorrow = false) {
  ...
  const weatherUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${weatherApiKey}&units=metric&lang=ja`;
  ...
  try {
    ...
    const forecast = res.data.list.find(
      (item) => item.dt_txt.startsWith(targetDay) && item.dt_txt.includes('06:00')
    );

    const dateStr = `${targetDate.getMonth() + 1}月${targetDate.getDate()}日（${getWeekdayName(
      targetDate
    )}）`;
    const weather = forecast ? forecast.weather[0].description : '不明';
    const windSpeed = forecast ? forecast.wind.speed : 0;
    const waveHeight = await getWaveHeight(targetDay);

    const tide = '中潮'; // 仮固定（→のちにCSVから取得）
    const highTideTime = '06:30'; // 仮固定（→のちにCSVから取得）
