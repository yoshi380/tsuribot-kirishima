...
const tideData = JSON.parse(fs.readFileSync('./tideData.json', 'utf8'));
...
async function getFishingInfo(isTomorrow = false) {
  ...
  try {
    ...
    const forecast = res.data.list.find(
      (item) => item.dt_txt.startsWith(targetDayStr) && item.dt_txt.includes('06:00')
    );
    ...
    const tide = '中潮'; // 仮固定
    const highTideTime = tideData[targetDayStr] || '06:30'; // JSONから取得
